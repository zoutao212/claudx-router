...
function App() {
  ...
  const refreshRuntimeMetrics = useCallback(async () => { try { const next = (await window.desktop.getRuntimeMetrics()).metrics; if (next) setRuntimeMetrics(next); } catch { /* Service may be restarting. */ } }, []);
  ...
  useEffect(() => { void refreshStatus(); void refreshRuntimeEvents(); void refreshRuntimeMetrics(); void load(); const timer = window.setInterval(() => { void refreshStatus(); void refreshRuntimeEvents(); void refreshRuntimeMetrics(); }, 1_000); return () => window.clearInterval(timer); }, [load, refreshRuntimeEvents, refreshRuntimeMetrics, refreshStatus]);
  ...
  return <div className="app-shell">
    ...
    <div className="app-body"><aside className="side-nav"><button className={page === "overview" ? "nav-item nav-item--active" : "nav-item"} onClick={() => setPage("overview")}><LayoutDashboard size={18}/><span>总览</span></button><button className={page === "config" ? "nav-item nav-item--active" : "nav-item"} onClick={() => setPage("config")}><Settings2 size={18}/><span>配置工作台</span></button><div className="nav-spacer"/><button className="nav-item" onClick={() => void window.desktop.openLogs()}><FolderOpen size={18}/><span>日志目录</span></button></aside>
      <main className="page-scroll">{page === "overview" ? <section className="page"><div className="page-heading"><div><p className="eyebrow">LOCAL CONTROL PLANE</p><h1>服务总览</h1><p>CCR 的运行状态与请求指标会在这里汇总。</p></div><div className="page-actions">{running ? <button className="button button--danger" disabled={busy} onClick={() => void execute(() => window.desktop.stop())}><Square size={16}/>停止服务</button> : <button className="button button--primary" disabled={busy} onClick={() => void execute(() => window.desktop.start())}><Play size={16}/>启动服务</button>}<button className="button button--secondary" disabled={busy} onClick={() => void execute(() => window.desktop.restart())}><RefreshCw size={16}/>重启</button></div></div>
        <section className="metrics"><article><span>缓存命中率</span><strong>{runtimeMetrics?.cacheHitRate == null ? "—" : `${(runtimeMetrics.cacheHitRate * 100).toFixed(2)}%`}</strong><small>{runtimeMetrics?.tokens.reportedRequests ? `缓存读取 ${formatNumber(runtimeMetrics.tokens.cacheReadTokens)} Token` : "等待上游返回 usage"}</small></article><article><span>对话轮次</span><strong>{formatNumber(runtimeMetrics?.requests.total ?? 0)}</strong><small>有效 {formatNumber(runtimeMetrics?.requests.successful ?? 0)} / 异常 {formatNumber(runtimeMetrics?.requests.failed ?? 0)}{runtimeMetrics?.requests.active ? ` / 进行中 ${formatNumber(runtimeMetrics.requests.active)}` : ""}</small></article><article><span>Token 消耗</span><strong>{formatNumber(runtimeMetrics?.tokens.totalTokens ?? 0)}</strong><small>Prompt {formatNumber(runtimeMetrics?.tokens.promptTokens ?? 0)} / Completion {formatNumber(runtimeMetrics?.tokens.completionTokens ?? 0)}</small></article><article><span>价值估算</span><strong>—</strong><small>未配置模型价格 · 缓存写入 {formatNumber(runtimeMetrics?.tokens.cacheWriteTokens ?? 0)} Token</small></article></section>
